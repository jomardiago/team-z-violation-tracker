...
export class ViolationInputFormComponent implements OnInit {
  ...
  startDateChanged() {
    ...
    var endDate = startDate['year'] + "-" + ((startDate['month'] + 6) < 12 ? '0' + (startDate['month'] + 6) : (startDate['month'] + 6)) + "-" + (startDate['day'] < 10 ? startDate['day'] : '0' + startDate['day']);
    this.selectedViolation.endDate = endDate;
  }

  saveViolation() {
    var startDate = this.selectedViolation.startDate
    var startDateString = startDate['year'] + "-" + (startDate['month'] < 12 ? '0' + startDate['month'] : startDate['month']) + "-" + (startDate['day'] < 10 ? startDate['day'] : '0' + startDate['day']);
    var newObject = {
